...
object AST {
	...
	case class TypeAssign(tpe : Type, ident: Ident, assignRHS : AssignRHS) 
		extends Stat
	case class AssignLR(assignLHS : AssignLHS, assignRHS : AssignRHS) 
		extends Stat
	...
	case class If(expr : Expr, statThen : List[Stat], statElse : List[Stat]) 
		extends Stat
	case class While(expr : Expr, stat : List[Stat]) extends Stat
	case class Begin(stat: List[Stat]) extends Stat
	
	sealed case class Ident(string: String) 
		extends AssignLHS with AssignRHS with Expr {
	...
	sealed case class ArrayElem(ident: Ident, exprList : List[Expr]) 
		extends AssignLHS with Expr {
		override def getType(symbTable: SymbolTable): Type = {
	...
	case class Fst(fst : Expr) extends PairElem {
		...
		override def getType(symbTable: SymbolTable): Type = {
			fst match {
				case Ident(_)	=> 
					val fstType = fst.getType(symbTable)
					fstType match {
						case Pair(fst, _) => 
							return fst.getType
						case _ => 
	...
	case class Snd(snd : Expr) extends PairElem {
		...
		override def getType(symbTable: SymbolTable): Type = {
			snd match {
				case Ident(_) => 
					val sndType = snd.getType(symbTable)
					sndType match {
	...
	sealed trait UnOp extends Expr {
		...
		override def getType(symbTable: SymbolTable): Type = {
			...
			if (current != expected._1) {
				semanticErrors += 
					MismatchTypesErr(expr, current, List(expected._1))
			}
			expected._2
		}
	...
	sealed trait BinOp extends Expr {
		...
		override def getType(symbTable: SymbolTable): Type = {
			...
			semanticErrors += 
				MismatchTypesErr(exp2, currentExp2, expected._1)
			expected._2
		}
		override def toString: String = 
	...
	case class DerefPointer(ptr: Expr) extends Expr with AssignLHS {
		override def getType(symbTable: SymbolTable): Type = {
			...
			tpe match {
				...
				case _                   =>
				semanticErrors += 
					MismatchTypesErr(ptr, tpe, List(PointerType(null)))
				null
			}
		}
	}
	...
	case class Realloc(ptr: Ident, size: Expr) extends Heap {
		override def getType(symbTable: SymbolTable): Type = {
			...
			if (tpe != Int) {
				semanticErrors += 
					MismatchTypesErr(size, tpe, List(Int))
			}
			val ptrTpe = ptr.getType(symbTable)
			semanticErrors ++= ptr.semanticErrors
			if (ptrTpe != PointerType(null)) {
				semanticErrors += 
					MismatchTypesErr(ptr, ptrTpe, List(PointerType(null)))
			}
			ptrTpe
		}
	}
