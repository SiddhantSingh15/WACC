...
object WACCErrors {
    ...
    case class TestError(pos: (Int, Int), lines: TestErrorLines, errorMessage: String) {
    ...
    class TestErrorBuilder extends ErrorBuilder[TestError] {
        override def format(pos: Position, source: Source, lines: ErrorInfoLines): TestError = TestError(pos, lines, createString(pos, lines))

        def createString(pos: Position, lines: ErrorInfoLines) : String = {
        val sb = new StringBuilder(s"Error found at line ${pos._1} and column ${pos._2 -1}\n\n")
        lines match {
