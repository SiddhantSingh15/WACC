...
sealed trait SemanticError {
  override def toString: String = this match {
    case MismatchTypesErr(invalid, actualT, expected) =>
      "Type mismatch at " + invalid + " expected type " + 
        expected.mkString( " or " ) + " but actual type " + actualT
    case OutOfBoundErr(ident) => 
      "Array index out of bounds for array " + ident
    case AccessDeniedErr(ident) => 
      "Element access of " + ident + " is not permitted"
    case InvalidRetErr(e) => 
      "Invalid return statement from main: return " + e
    case IllegalAssignFuncErr(ident) => 
      "Illegal assignment to function " + ident
    case InvalidParamsErr(ident, actual, expected) =>
      "Invalid params for function " + ident + " Expected number of params: " + 
        expected + ". Actual: " + actual
    case InvalidPairElemErr(pe) =>
      "Invalid input, expected: Pair, unable to perform " + pe
    case DeclaredFuncErr(ident)    => 
      DeclaredIdentErr(ident).toString()
    case NotDeclaredFuncErr(ident) => 
      NotDeclaredIdenErr("Function", ident).toString
    case NotDeclaredVarErr(ident) => 
      NotDeclaredIdenErr("Variable", ident).toString()
    case DeclaredVarErr(ident)    => 
      DeclaredIdentErr(ident).toString()
    case NotDeclaredIdenErr(t, ident) =>
      t + " " + ident + " not declared in current scope"
    case DeclaredIdentErr(ident) => 
      "Conflicting definitions for variable " + ident + "."
    case IllegalFree(expr) => 
      expr.toString + " is not available to free."
    case FuncNoRetErr(ident) => "Function \'" + ident.toString + 
      "\' does not exit with " + "{return, if, while, exit}" 
    case IllegalReference(expr) => 
      "Invalid memory access (check & or * usage)."
  }
}

case class MismatchTypesErr(invalid: AssignRHS, actualT: Type, 
  expected: List[Type]) extends SemanticError
